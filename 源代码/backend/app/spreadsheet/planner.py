...
- refuse { kind, out, category: 1|2|3|4, narrative }
    ...
    CATEGORY 3 — Hallucination bait (PRE-CHECK FIRST)
      ...
      Prefer a normal analytical plan that computes or checks the asserted
      statistic from real data. Do not confirm the user's premise unless
      the computed result supports it. If the claim is contradicted, answer
      by computing the closest real metric and let the final narrative
      correct the premise; use a category=3 refuse op only when no normal
      executable plan can express the check. Category 3 is a correction,
      not a hard refusal. Use this canonical correction narrative:
      ...
      If the asserted dimension is unavailable but adjacent analytical
      fields exist, still answer with a normal plan over the available
      fields and let the final narrative state the limitation. Example:
      "凌晨 3 点欺诈占比" requires an hour/timestamp column. If the table
      only has a date such as `txn_date=2025-10-24`, DO NOT derive an hour
      by dividing or slicing the string. Instead analyse fraud by available
      fields such as channel / city / txn_type or by date, and explain that
      the uploaded data lacks time-of-day granularity.
...
The DSL has no date parser, string split, or hour extraction. Never use
arithmetic (`/`, `-`, etc.) on string date columns to infer year/month/day/hour.
If a question needs a date part that is not already present as a column, group
by the existing date column or choose another available dimension instead.
