...
class StageTimer:
    ...
    # Per-stage callback. Invoked synchronously inside `record(stage)` with
    # the stage name and its duration in seconds. The streaming
    # `/v1/analyze/stream` endpoint uses this to push live progress events
    # into an asyncio.Queue without restructuring the handler — the legacy
    # `/v1/analyze` path leaves it None, paying nothing.
    listener: Callable[[str, float], None] | None = field(default=None)
    ...
    def record(self, stage: str) -> None:
        ...
        duration = max(0.0, now - self._last)
        self.durations[stage] = duration
        self._last = now
        # Telemetry must never break the request — swallow listener
        # exceptions. The streaming endpoint's queue.put_nowait can't fail
        # in practice (unbounded queue), but a future bounded variant or a
        # buggy custom listener shouldn't bubble into the handler.
        if self.listener is not None:
            try:
                self.listener(stage, duration)
            except Exception:  # nosec - intentional: telemetry side-channel
                pass
...
def bind_stage_timer(
    *, listener: Callable[[str, float], None] | None = None
    ...
    `listener` is the optional per-stage callback used by the streaming
    `/v1/analyze/stream` endpoint to receive (stage_name, duration_s)
    tuples as each stage completes. The non-streaming endpoint omits
    it and pays nothing.
    """
    timer = StageTimer(listener=listener)
