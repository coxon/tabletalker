...
class Chart(_StrictModel):
    ...
    `echarts_option` is the JSON-encoded ECharts option (already
    HTML-safe) for clients that want to render the chart inline
    without iframing the full report. Optional / nullable so the
    contract stays additive — older clients that ignore the field
    still work via `html_anchor` + the report iframe.
    """
    ...
    echarts_option: str | None = Field(
        default=None,
        description=(
            "JSON-encoded ECharts option — the same payload embedded "
            "into the HTML report. Lets the SPA render the chart "
            "directly in the chat thread instead of opening the "
            "report side-panel. Empty string / null = no chart "
            "(refusal turn or non-renderable answer)."
        ),
    )
...
class AnalyzeResponse(_StrictModel):
    ...
    suggested_questions: list[str] = Field(
        ...
        description=(
            "0-3 follow-up questions the user can click to keep the "
            "conversation going. Generated by the finalize LLM from the "
            "current findings + execution context. Frontend renders them "
            "as chips below the assistant turn (claude.ai-style)."
        ),
    )
