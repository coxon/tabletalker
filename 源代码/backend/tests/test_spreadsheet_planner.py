...
def test_planner_prompt_forbids_fake_hour_extraction_from_date_strings() -> None:
    """The bank hallucination-bait trap only has txn_date, not hour.

    The planner prompt must steer the model away from string arithmetic
    such as dividing "2025-10-24" to invent a 3am bucket.
    """

    assert "time-of-day" in _PLANNER_PROMPT
    assert "DO NOT derive an hour" in _PLANNER_PROMPT
    assert "Never use\narithmetic" in _PLANNER_PROMPT
    assert "Category 3 is a correction" in _PLANNER_PROMPT
