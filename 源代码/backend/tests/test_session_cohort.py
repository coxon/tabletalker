...
def _ev(
    *,
    ...
    return Evidence(
        ...
        value=value if value is not None else (row_count if row_count is not None else 0),
        row_count=row_count,
    )
...
def test_extract_cohorts_uses_filtered_group_count_after_global_count() -> None:
    """Top-N evidence starts with the table size, then the winning group.

    The winning group is the scope that follow-up pronouns like "其中" need.
    """
    ...
    findings = [
        _finding(
            "美国产量占据绝对主导",
            [
                _ev(filters="", row_count=4803, aggregation="count(*)"),
                _ev(
    ...
    assert len(cohorts) == 1
    assert cohorts[0].name == "美国产量占据绝对主导"
    assert cohorts[0].filters == "production_countries == 'United States of America'"
    assert cohorts[0].row_count == 3956
