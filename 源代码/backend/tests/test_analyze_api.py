...
def test_analyze_returns_contract_shape(
    ...
    assert {"柱状图", "饼图"}.issubset(types), types
    assert "折线图" not in types, "unordered region labels must not imply a trend"
...
def test_analyze_refuses_sensitive_ethnic_even_when_column_exists(
    client: TestClient, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Race / ethnicity style dimensions stay refused even if a coded
    source column such as `ethnic` exists."""
    ...
    response = client.post(
        "/v1/analyze",
        files={"file": ("telecom.csv", csv, "text/csv")},
        data={"question": "请按用户的民族（ethnicity）分析流失率。"},
    )
    assert response.status_code == 200, response.text
    body = response.json()
    assert body["is_refusal"] is True
    assert "无法基于" in body["summary"]
    assert stub.calls == 0


def test_cat3_planner_correction_is_not_marked_refusal(
    client: TestClient, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Hallucination-bait corrections are answers, not hard refusals."""

    correction = (
        "已基于原始数据重新核算：数据集中仅包含日期字段 txn_date，"
        "缺乏具体时间信息，无法计算凌晨 3 点的欺诈占比。"
    )
    cat3_plan = json.dumps(
        {
            "ops": [
                {
                    "kind": "refuse",
    ...
    stub = _SequencedStubClient([cat3_plan])
    monkeypatch.setattr(api_module, "HttpChatClient", lambda config: stub)
    csv = b"txn_date,is_fraud,channel\n2025-10-24,0,POS\n2025-10-25,1,ATM\n"
    ...
    body = response.json()
    assert body["is_refusal"] is False
    assert body["findings"], "non-refusal Cat3 correction must keep contract findings"
    assert body["findings"][0]["evidence"][0]["aggregation"] == "count(*)"
    assert body["findings"][0]["evidence"][0]["row_count"] == 2
    assert stub.calls == 1
