      ...
      <div className="xl:col-span-2 space-y-6">
        ...
        {!parent ? (
          ...
          <>
              ...
              <div className="mb-6">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="flex items-center gap-4 text-xs">
                    <span className="text-slate-400">已上传 {files.length} 个文件</span>
                    <button onClick={() => setFiles([])} className="text-indigo-600 flex items-center gap-1 hover:text-indigo-700 transition-colors">
                      <X size={14} /> 清空
                    </button>
                  </div>
                </div>
                <p className="text-xs text-slate-400 mb-4 ml-8">支持 CSV、Excel 格式，单文件最大 200MB；多文件时第一个为主表，其余作为附表参与 join</p>

                <div className="ml-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  {files.map((f, idx) => (
                    <div key={`${f.name}-${idx}`} className="flex items-center justify-between p-3 rounded-lg border border-indigo-200 bg-indigo-50/50">
                      <div className="flex items-center gap-3 overflow-hidden">
                        <FileSpreadsheet className="text-emerald-500 shrink-0" size={20} />
                        <div className="min-w-0">
                          <p className="text-sm font-medium text-slate-700 truncate">
                            {idx === 0 && files.length > 1 && (
                              <span className="text-[10px] mr-1.5 px-1.5 py-0.5 rounded bg-indigo-100 text-indigo-700 font-semibold">主表</span>
                            )}
                            {f.name}
                          </p>
                      ...
                      <button onClick={() => removeFile(idx)} className="text-slate-400 hover:text-slate-600 p-1">
                        <X size={16} />
                      </button>
                    </div>
                  ))}
                  ...
                  >
                    ...
                    <span className="text-xs text-slate-400 mt-1">或将文件拖到此处（可多选）</span>
                    <input
                      type="file"
                      ref={fileInputRef}
                      className="hidden"
            ...
            {/* Live progress: first-submit case shows timeline below the
                upload form (the conversation branch isn't mounted yet
                because there are no turns yet). */}
            {phase.name === "analyzing" && (
              <motion.div
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
              >
                <StageTimeline stages={stages} inFlight={true} />
              </motion.div>
            )}
          ...
          <div className="space-y-6">
            ...
            <AnimatePresence initial={false}>
              ...
              {/* Live progress: when analyzing, show stage timeline driven
                  by the NDJSON event stream. Follow-up keeps the simple
                  spinner because that path doesn't (yet) stream stages. */}
              {phase.name === "analyzing" && (
                <motion.div
                  initial={{ opacity: 0, y: 10 }}
                  animate={{ opacity: 1, y: 0 }}
