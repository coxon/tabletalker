...
| 3.5 | `feat/spreadsheet-engine` | 5/6 | Internal typed-plan engine (parked from earlier exploration); reused by PR #4 as the audit/trace layer | ☐ |
| 4 | `feat/analyze-pipeline` | 5/7 | Profiler → ReAct planner → pandas sandbox → evidence extraction → JSON contract response | ☐ |
