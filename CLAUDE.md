...
1. **提交目录必须保留。** 仓库根目录必须包含：
   `源代码/`、`架构文档/`、`运行脚本/`、`演示视频/`、`自测报告/`。
   `docs/` 和 `knowledge-base/` 是工程补充目录；测试在 `源代码/backend/tests/`。
...
3. **宁可拒答，不要幻觉。** 如果结论无法从上传数据复算，报告必须说明原因。
   ...
   `源代码/backend/tests/test_analyze_api.py` 和
   `源代码/backend/tests/test_followup_api.py`。
